...
if ($CheckOnly) {
    & (Join-Path $ProjectRoot '.venv\Scripts\python.exe') -c "from src.rtmw3d.runtime import default_readiness; import json; r=default_readiness(); print(json.dumps({'ready':r.ready,'missing':r.missing_files,'environment':str(r.environment),'repository':str(r.repository)}))"
    exit $LASTEXITCODE
}
